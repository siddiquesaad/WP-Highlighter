...
class EDD_SL_Plugin_Updater {
	...
	public function init() {
		...
		// Divi override, see function description
		add_filter( 'gettext', array( $this, 'edd_update_notifications' ), 1, 3 );

	}

	/**
	 * Hacks the Divi update plugins gettext filters very early to fix divi hijacking update not available core string on
	 * /public_html/wp-content/themes/Divi/core/admin/includes/class-updates.php line 44 function array( $this, 'update_notifications' ), 20, 3
	 ...
	 */
	function edd_update_notifications( $default_translated_text, $original_text, $domain ) {
		$message = '';
		...
		if ( is_admin() ) {
			if ( in_array( $original_text, $theme_plugin_updates_unavailable, true ) ) {
				$message = __( 'Automatic updates currently unavailable. Please authenticate your license.', 'easy-digital-downloads' );
			}
